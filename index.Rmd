...
tab = rbind( d_i,s_i,p_right_tail )
tab_df = data.frame(tab)
colnames(tab_df) <- 1:4
knitr::kable(tab_df, digits = 3)
```

Applying the first step of hypothesis selection based on the direction favored by the data,
we select the hypotheses $H_1^-$, $H_2^+$, $H_3^+$, $H_4^+$ and the corresponding $p$-values:
...
RES
```
Adaptive partitioning procedure: $95\%$ confidence lower and upper bounds with the Fisher's combining function on the number of studies with positive effect in the (data-driven) top $k$ schools:
...
GuoRomano2015 <- function(p_right_tail, alpha=0.05){
  ...
  p <- p_right_tail
  n <- length(p)
  sp2 <- sort( c(p,1-p), index.return=TRUE )
  REJ <- sp2$x <= alpha / (n - 1:n + 1L + alpha)
  REJ_NONPOS <-  1:n %in% sp2$ix[REJ]
  REJ_POS <-  (n+1):(2*n) %in% sp2$ix[REJ]
