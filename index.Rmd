...
for (i in 1:4){
 ...
 res_dct_lo <- length(S_minus) - sum(nplus_pvalue(p_right_tail, method=RES[2*i,1], ix = S_minus) > alpha) + 1
 res_dct_up <- n - (length(S_plus) - sum(nplus_pvalue(p_right_tail, method=RES[2*i,1], ix = S_plus) > alpha) + 1) 
